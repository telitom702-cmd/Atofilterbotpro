...
def normalize_bengali_numbers(text):
    """Convert Bengali numerals to English numerals for better search."""
    bengali_to_english = {
        '০': '0', '১': '1', '': '2', '৩': '3', '৪': '4',
        '৫': '5', '৬': '6', '৭': '7', '৮': '8', '৯': '9'
    }
...
async def save_file(media):
    """Save file in database with Unicode/Bangla support."""
    
    file_id, file_ref = unpack_new_file_id(media.file_id)
    
    # মূল নাম সংরক্ষণ - শুধু এক্সটেনশন রিমুভ করুন
    original_name = str(media.file_name)
    
    # ফাইল এক্সটেনশন (.mkv, .mp4, .avi, etc.) রিমুভ করুন
    clean_name = re.sub(r'\.(mkv|mp4|avi|webm|mov|flv|m4v|wmv|3gp)$', '', original_name, flags=re.IGNORECASE)
    
    # একাধিক স্পেসকে একটি স্পেসে রূপান্তর করুন
    clean_name = re.sub(r'\s+', ' ', clean_name).strip()
    
    # যদি নাম খালি হয়ে যায়, অরিজিনাল নাম ব্যবহার করুন
    if not clean_name:
        clean_name = original_name
    
    # Mime Type এবং File Type ফিক্স
    ...
    # যদি ডকুমেন্ট হয় কিন্তু ভিডিও/অডিও মাইম টাইপ হয়
    if media.file_type == "document" and media.mime_type:
    ...
    if MULTIPLE_DB:
        try:
            ...
            if exists:
                logger.info(f"[SKIP] '{clean_name[:50]}...' already in Primary DB.")
                return False, 0
    ...
    try:
        ...
        logger.exception(f"[VALIDATION ERROR] '{clean_name[:50]}...' → {e}")
        return False, 2
        
    try:
        await record.commit()
        logger.info(f"[SUCCESS] '{clean_name[:60]}...' saved to {target_db} DB.")
        return True, 1
    except DuplicateKeyError:
        logger.info(f"[SKIP] DuplicateKey: '{clean_name[:50]}...' already exists in {target_db} DB.")
        return False, 0
    except Exception as e:
        logger.exception(f"[ERROR] Failed commit of '{clean_name[:50]}...' to {target_db} DB.", exc_info=e)
        return False, 3


async def get_search_results(chat_id, query, file_type=None, max_results=None, offset=0, filter=False):
    """Search with Unicode/Bangla and partial match support."""
    ...
    # বাংলা সংখ্যাকে ইংরেজিতে কনভার্ট করুন
    query_normalized = normalize_bengali_numbers(query) if isinstance(query, str) else query
    
    # ইউনিকোড-সচেতন রেজেক্স প্যাটার্ন
    if isinstance(query_normalized, list):
        raw_pattern = '|'.join(re.escape(q.strip()) for q in query_normalized if q.strip())
        regex_list = [re.compile(raw_pattern, re.IGNORECASE | re.UNICODE)] if raw_pattern else []
        ...
        # Partial match - যেকোনো জায়গায় query থাকলেই ম্যাচ করবে
        raw_pattern = f".*{re.escape(query_normalized)}.*"

        try:
            regex = re.compile(raw_pattern, flags=re.IGNORECASE | re.UNICODE)
