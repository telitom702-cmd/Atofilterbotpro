...
from info import ADMINS, INDEX_REQ_CHANNEL as LOG_CHANNEL, FILE_STORE_CHANNEL
...
# নতুন মিডিয়া টাইপ যোগ করা হলো যেগুলো আগে স্কিপ হতো
...
def get_size(size):
    ...
    i = 0
    while float_size >= 1024.0 and i < len(units):
        i += 1
        float_size /= 1024.0
    return f"{round(float_size, 2)} {units[i]}"
...
# ==========================================
# ফিল্টার এবং সার্চ হ্যান্ডলার (Search Filter)
# ==========================================

# ডাটাবেস থেকে ফাইল খোঁজার ফাংশন (আপনার ডাটাবেস স্ট্রাকচার অনুযায়ী কাজ করবে)
async def search_files(query, max_results=10):
    from database.ia_filterdb import col  # আপনার ডাটাবেস কালেকশন ইম্পোর্ট করা হলো
    import re
    regex = re.compile(query, re.IGNORECASE)
    results = []
    cursor = col.find({"file_name": {"$regex": regex}}).limit(max_results)
    async for doc in cursor:
        results.append(doc)
    return results

# প্রাইভেট চ্যাটে ইউজারের মেসেজ ধরার হ্যান্ডলার
@Client.on_message(filters.private & filters.text & ~filters.edited & ~filters.command(['start', 'setskip']))
async def filter_search(bot, message):
    ...
    files = await search_files(query, max_results=10)
    
    if not files:
        return await wait_msg.edit(
            f"❌ <b>No results found for:</b> <code>{query}</code>\n\n"
            f"Try searching with a different keyword or correct spelling."
        )
        ...
        buttons.append(
            [
                InlineKeyboardButton(
                    text=f"📂 {file_name[:50]} ({file_size})", 
                    url=f"https://t.me/{bot.username}?start=files_{message_id}"
                )
            ]
        )
    
    await wait_msg.edit(
        f"✅ <b>Found {len(files)} results for:</b> <code>{query}</code>\n\n"
        f"Click on the buttons below to get your files.",
        reply_markup=InlineKeyboardMarkup(buttons)
    )
