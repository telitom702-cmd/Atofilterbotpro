    ...
    async with lock:
        try:
            ...
            for batch in range(batches):
                ...
                for message in messages:
                    ...
                    try:
                        if message.empty:
                            ...
                            no_media += 1
                            continue
                        # ভিডিও, অডিও, ডকুমেন্ট এবং অ্যানিমেশন (GIF) সাপোর্ট
                        elif message.media not in [enums.MessageMediaType.VIDEO, 
                                                    enums.MessageMediaType.AUDIO, 
                                                    enums.MessageMediaType.DOCUMENT,
                                                    enums.MessageMediaType.ANIMATION]:
                            unsupported += 1
                            logger.debug(f"Unsupported media type: {message.media} for msg ID: {message.id}")
                            continue
                        ...
                        if message.media == enums.MessageMediaType.DOCUMENT and hasattr(media, 'mime_type') and media.mime_type:
                ...
                percentage = (progress / total_fetch) * 100 if total_fetch > 0 else 0
